...
- **Required sections** — Every task file must include: Description, Dependencies, Threats Mitigated, Technical Requirements, Acceptance Criteria, and Testing Requirements.
...
- **Threat traceability** — `Threats Mitigated` lists every `THR-NNN` from `specs/threat-model.md` whose required mitigation this task delivers, or `None` with rationale when the task implements no mitigation. Each cited threat must map to at least one acceptance criterion and one mitigation test.
